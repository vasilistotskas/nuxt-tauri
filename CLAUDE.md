...
The monorepo has multiple `package.json` files, each with a distinct role:

- **Root `package.json`** — Bun workspace definition (`workspaces: ["packages/*", "apps/*"]`), root-level scripts (`lint`, `typecheck`, `app`, `new-brand`, `postinstall`), shared devDependencies (eslint, typescript, nuxt, vue-tsc), and engine requirements (`node >= 24`).
- **`packages/core/package.json`** — Declares the core layer's runtime dependencies (`@nuxt/ui`, `@nuxtjs/i18n`, `@pinia/nuxt`, `pinia`, `zod`, etc.) that brand apps inherit via `"@packages/core": "workspace:*"`. Sets `"main": "./nuxt.config.ts"` so Nuxt resolves it as a layer.
- **`apps/<brand>/package.json`** — Brand app dependencies (core layer reference, Tauri JS plugins), brand-specific scripts (`dev`, `build`, `tauri:*`, `web:*`). Generated by `scripts/new-brand.ts`.
...
- Module: `@nuxtjs/i18n` v10 configured in `packages/core/nuxt.config.ts` (inherited by all brands)
- Default locale: English (`en`), also supports Greek (`el`)
- Strategy: `prefix_except_default` (English URLs have no prefix, Greek gets `/el/`)
- Lazy-loaded locale files in JSON format (`restructureDir` defaults to `i18n`, `langDir` defaults to `locales`)
...
- `apps/<brand>/i18n/locales/{en,el}.json` — brand-specific global translations (merged with core via layer system)

**Layer locale merging:** Brand apps **must** declare `i18n.locales` in their `nuxt.config.ts` to trigger locale file merging with the core layer. Without this, only core translations are loaded. The module deep-merges JSON files with the same name from each layer, with the brand app taking priority for duplicate keys.

```ts
// apps/<brand>/nuxt.config.ts
i18n: {
  locales: [
